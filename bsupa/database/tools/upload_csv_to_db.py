...
def appendReadingDict(ssid_dict, reading):
    ...
    """
    for ap_id, signal_strength in reading['i'].items():
...
def convertToCells(list_readings):
    ...
    while list_readings != []:
        ...
        for each_reading in list_readings:
            ...
            if each_reading['x'] == first_reading['x'] and each_reading['l'] == first_reading['l']:
                # Append readings
                ssid_dict = appendReadingDict(ssid_dict, each_reading)
                if sum(each_reading['g']) != 0:
                    gps       = each_reading['g']
                    timestamp = each_reading['t']

                thrash.append(each_reading['t'])

        # New List contains all the reaadings which dont have first_reading's coords
        new_list = []
        # Clean up
        for each_reading in list_readings:
            if not each_reading['t'] in thrash:
        ...
        if timestamp != -1:
            ...
            cell['timestamp'] = first_reading['t']

        cell['hash']      = "eeLKSGLUERG4"
        cell['section']   = first_reading['l']
        cell['floor']     = first_reading['F']#none
        cell['property']  = first_reading['M']#none
        cell['model']     = first_reading['m']#none
        cell['coords']    = [first_reading['x']
...
def main():
    ...
    """
    dat_path = "./blip/database/tools/test_live_data/"
