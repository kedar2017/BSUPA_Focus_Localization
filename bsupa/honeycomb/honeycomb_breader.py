...
class HoneycombPredict:
	...
	def predictCells(self,msg):
		...
		for key in msg.keys():
			...
			cell.rssi_obj.mean    = statistics.mean(msg[key])
                        if len(msg[key])!=1 and len(msg[key])!=0:
        			cell.rssi_obj.std     = math.sqrt(statistics.variance(msg[key]))
                        else:
                                cell.rssi_obj.std     = 0.0
			cell.add_rssi()
		return cell
	...
	def getPsDict(self, shop_list):
		'''
		Get BLE position dict with shop name
		'''
		shop_dict = {}
		for j in range(len(shop_list)):
	...
	def getTraining(self,shop_list):
		...
		for j in range(len(shop_list)):
			...
			for cid in cid_shop:
				temp_dict = {}				
				for col1,col2,col3 in self.training_dbo.readTableWithCondition('processed_data','ble_id,mean,std',[cid]):       #returns UUID, mean, sigma corr. to one row of given CID
					for size in range(len(col1)):
						temp_dict[col1[size]]=(col2[size],col3[size])   #Temp dict is a dictionary of UUIDs as key and (mean, sigma) as value for given location
				train_dict.append([temp_dict,sect_shop[count],0])               #train_dict is a list of [temp_dict, section,0]) ??
				count = count + 1
			...
			ble_dict[shop_list[j]] = train_dict                                     #ble_dict is a dict - {shopname:train_dict}
			print "TRAINING", train_dict
			train_pos[shop_list[j]]= (list_x, list_y)                               #train_pos is a dict - {shopname:(xlocations _mapped, locations _mapped}
